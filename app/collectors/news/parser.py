...
def _extract_meta_content(
    ...
    if property_name:
        ...
        if tag and tag.get("content"):
            return _clean_html_text(tag["content"])
...
def parse_article_html(
    ...
    if title_tag:
        title = _clean_html_text(title_tag.get_text())
    ...
    if not title or title == "Yahoo Finance":
        title = _clean_html_text(fallback_title)

    if not title:
        fallback_title_tag = soup.find("title")
        title = _clean_html_text(fallback_title_tag.get_text()) if fallback_title_tag else None
    ...
    if article:
        for p in article.find_all("p"):
            text = _clean_html_text(p.get_text())
    ...
    if not published_at:
        ...
        if meta_time:
            published_at = meta_time.get("datetime") or _clean_html_text(meta_time.get_text())
    ...
    if not summary:
        summary = _clean_html_text(fallback_summary) or None
